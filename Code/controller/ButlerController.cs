...
namespace Project
{
    class ButlerController
    {
        ...
        {
            Console.WriteLine("Le Task (thread) WatchLoop du controlleur ds tables du restaurant est appelé");
                        ...
                        foreach (Square square in model.restaurationRoom.squares)
                        {
                            foreach (Rank rank in square.ranks)
                            {
                                foreach (Table table in rank.tables)
                                {
                                    ...
                                    {
                                        Console.WriteLine("la table " + table.numberTable + " a été assigné"); //attribution d'une tabe au client
                                        table.avaible = false;
                                        table.client = client;
                                        //Added
